    ...
    def __send(self, ):
        ...
        try:
            ...
            for i in range(6):
                ...
                if self.__get_proxy() and i < 5:

                    proxy = self.__proxies.get('http')
                ...
                if not response.text:
                    if self.__trace_log(False, '获取代码为空![%s]'%proxy): continue

                # 页面出现验证码
                if 'Type the characters you see in this image:' in response.text:

                    self.__trace_log(False, '出现验证码![%s]'%proxy)
                    continue
            ...
            if self.__trace_log(False, '产品页HTTP请求错误! 代理地址:[%s]' % proxy): return False
