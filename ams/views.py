...
def login():
    if request.method == 'POST':
        ...
        user = User.query.filter(User.name == username, User.pwd == password).one()
...
def index():
    ...
    data = db.session.query((Listing.id).label('aid'), Listing.img, Listing.asin, Listing.adtime, ssub).join(ssub, isouter=True).order_by(Listing.adtime.desc()).all()

    ss = {'title': title, 'data': data}
...
def lists():
    ...
    # ssb = db.session.query(Marks).filter(Marks.asin_id == id).all()
    #
    # print(ssb)
